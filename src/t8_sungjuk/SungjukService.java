...
//		scanner.close();	//service에서 close하면 Run에서 닫힌다.???????????? 
	...
	public void setList() {	//성적 전체 조회
		...
		//dao.getList() => DAO에서 반환받은 vos이다. 이것을 ArrayList<SungjukVO>라는 타입의 vos 변수에 담는다. '얕은복사'이다. new 생성하지 않고 그냥 씀.
		//'얕은복사'로 사용 가능한 경우..? //생성을 해야할 때 = 덮어쓰면 안 될 때..
		...
		for(int i=0; i<vos.size(); i++) {
			vo = vos.get(i); // ArrayList<SungjukVO>타입의 vos를 vo로 꺼냄
			calculator(vo);
