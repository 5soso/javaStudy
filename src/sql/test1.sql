...
-- 정렬 : order by 필드명 _옵션__     :  옵션생략시는 오름차순(asc), 내림차순(desc)
-- 성명 오름차순으로 출력?
...
-- 나이가 30대만 출력하시오?(30~39) : 필드명 between __ and __
...
-- 이름중에서 두번째글짜가 '나'자를 포함한 자료 출력?
select * from test1 where name like '_나%';

-- 이름중에서 두번째글짜가 '나'자를 포함한 자료의 '성명/주소'를 출력?
